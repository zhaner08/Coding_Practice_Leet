...
public class Solution {
    public int findRadius(int[] houses, int[] heaters) {
        ...
        for(int i = 0; i<houses.length; i++){
                while(second<heaters.length-1 && houses[i]>heaters[second])//need to reset number
                {
                    second ++;
                    first = second-1;
                }
            
            result=Math.max(result, Math.min(Math.abs(houses[i]-heaters[first]), Math.abs(houses[i]-heaters[second])));
        }
        return result;
    }
}
