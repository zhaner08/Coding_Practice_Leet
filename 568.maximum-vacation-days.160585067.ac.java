...
class Solution {
    ...
    public int maxVacationDays(int[][] flights, int[][] days) {
        ...
        int K = days[0].length;
        int max = 0;
        int[] dp = new int[N];//represent each possible city at different days, last day is the max of all
        Arrays.fill(dp, Integer.MIN_VALUE);//set to min value because if route is unavailable, never gonna compete with postive route
        dp[0] = 0;
        for(int i=0; i<K; i++){
            //we only care about previous day, so keep a tmp for new day
            int[] tmp = new int[N];
            Arrays.fill(tmp,  Integer.MIN_VALUE);
            for(int j=0; j<N; j++){
                for(int k=0; k<N; k++){
                    if(j==k || flights[j][k]==1){
                        tmp[k] = Math.max(tmp[k], dp[j] + days[k][i]);
                    }
                }
            }
            dp = tmp;
        }
        //last one contains answer
        for(int i : dp){
            max = Math.max(max, i);
        }
        return max;
