...
public class Solution {
    public int[] exclusiveTime(int n, List<String> logs) {
        ...
        for(int i=0; i<logs.size(); i++){
            //put start into the stack, when find end, take out one from stack since all end follow by a start.
            //Dont need to memorize the number and the word, since it is a valid log, save a split function
            String[] log = logs.get(i).split(":");
            if(!s.isEmpty()){
                result[s.peek()]+= Integer.parseInt(log[2])-previousTime;
                //update the previousTime
                previousTime = Integer.parseInt(log[2]);
            }
            if(log[1].equals("start")){
                //put which is the element to put into the stack
                //keep update the time when put a new number, every time a thread change, other threads stop, which we should stop counting
                s.push(Integer.parseInt(log[0]));
            }else{
                //if it is end, that time need to ++ since at is at the end of that second
                s.pop();
                result[Integer.parseInt(log[0])]++;
                previousTime++;
            }
        }
        return result;
