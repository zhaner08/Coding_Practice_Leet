...
class Solution {
    ...
    public List<Integer> boundaryOfBinaryTree(TreeNode root) {
        ...
        leftBDFS(backup.left, result);
        leaves(backup.left, result);
        leaves(backup.right, result);
        rightBDFS(backup.right, result, result.size());
        return result;
    }
    
    public void leftBDFS(TreeNode root, List<Integer> result){
        if(root == null) return;
        //if leaf
        if(root.left==null && root.right==null) return;
        result.add(root.val);
        if(root.left!=null){
             leftBDFS(root.left, result);
        }else{
            leftBDFS(root.right, result);
        }
    }
    ...
    public void rightBDFS(TreeNode root, List<Integer> result, int pos){
        if(root == null) return;
        //if leaf
        if(root.left==null && root.right==null) return;
        if(root.right!=null){
             rightBDFS(root.right, result, pos);
        }else{
            rightBDFS(root.left, result, pos);
        }
        result.add(root.val);
    }
}
