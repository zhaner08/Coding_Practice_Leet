...
public class Solution {
    public int leastBricks(List<List<Integer>> wall) {
        ...
        for(int i=0; i< wall.size(); i++){
            int tempSum = 0;
            for(int j =0; j<wall.get(i).size()-1; j++){//leave the right edge along, so only take size-1 breaks
                //record every breakthough, and and the line with the max break
                tempSum+=wall.get(i).get(j);
                result.put(tempSum, result.getOrDefault(tempSum, 0)+1);//Function only from Java8
                count = Math.max(count, result.get(tempSum));
            }
        }
        return size-count;
    }
