...
public class Solution {
    public int findMaxForm(String[] strs, int m, int n) {
        ...
        for (String s : strs) {
            int[] count = count(s);
            //for each string, update all possible value before either the count goes to negative
            for (int i=m;i>=count[0];i--) 
                for (int j=n;j>=count[1];j--) 
                    //if the current count already have number, compare if adding this number could get more, or the without this number, previous number in dp is already big enough
                    dp[i][j] = Math.max(1 + dp[i-count[0]][j-count[1]], dp[i][j]);
        }
        return dp[m][n];
    }

    //each string only go through once to count,
