...
class Solution {
    ...
    public int bestRotation(int[] A) {
        //K = (i - A[i] + N + 1) % N  :move that make it lose point
        //aiming index>=number
        //A[i] will be in the range [0, A.length]
        int length = A.length;
        int[] change = new int[length];
        for(int i=0; i<length; i++){
            change[(i-A[i]+1+length)%length]--; //if move this moves or more, it will lose point, more moves lose point anyway
        }
        int K = 0;
        for(int i=1; i<length; i++){
            //everytime we move 0 to length-1, we get one more point, no matter what since largest number will be smaller or equal to length-1, we dont need to track A[0]
            change[i] = change[i] + change[i-1] + 1; //move more steps, more losing point, but also some starts to get point
